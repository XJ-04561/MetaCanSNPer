...
class Minimap2(Mapper):
	...
	commandTemplate = "minimap2 -a {options} -R '@RG\\tID:{queryName}\\tSM:{queryName}' --sam-hit-only {refPath!r} {query} | samtools view -b -T {refPath!r} - | samtools sort -T {tmpDir!r} -o {output!r} - ; samtools index {output!r}"
	outFormat = "bam"
