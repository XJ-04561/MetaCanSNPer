...
class Minimap2(Mapper):
	...
	commandTemplate = "minimap2 -a {options} -R '@RG\\tID:{queryName}\\tSM:{queryName}' --sam-hit-only {refPath!r} {query} | samtools view -b -T {refPath!r} - | samtools sort -T {refName!r} -o {output!r} - ; samtools index {output!r}"
	outFormat = "bam"
	...
	def preProcess(self):
		...
		# 		# Get chromosome of this SNP
		# 		f.write(f"{self.Lib.chromosomes[0]}\t{pos-pad-2}\t{pos+pad}\n")
		# 	f.close()
		# if "Preset" in self.settings:
		# 	self.formatDict["options"] += ["--junc-bed", "{0[refPath]}.bed"]
		# 	'''Valid presets:
		# 	map-ont		Align noisy long reads of ~10% error rate to a reference genome. This is the default mode.
		# 	map-hifi	Align PacBio high-fidelity (HiFi) reads to a reference genome (-k19 -w19 -U50,500 -g10k -A1 -B4 -O6,26 -E2,1 -s200).
		# 	map-pb		Align older PacBio continuous long (CLR) reads to a reference genome (-Hk19).
