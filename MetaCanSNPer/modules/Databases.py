...
class DatabaseReader:
	...
	def _getSNPsByGenomeId(self, genomeID : int) -> sqlite3.Cursor:
		...
		LOGGER.debug(f'self._connection.execute("SELECT {SNP_COLUMN_SNP_ID}, {SNP_COLUMN_POSITION}, {SNP_COLUMN_ANCESTRAL}, {SNP_COLUMN_DERIVED} FROM {TABLE_NAME_SNP_ANNOTATION} WHERE {SNP_COLUMN_GENOME_ID} = ? ORDER BY {SNP_COLUMN_POSITION} ASC", {[genomeID]}) -> N={len(c.rowcount)}')
		return c
