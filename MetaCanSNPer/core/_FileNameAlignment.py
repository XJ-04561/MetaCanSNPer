...
def alignName(string : str, *strings : str, **kwargs):
    import os
    from This import this
    from typing import Generator

    if not strings and not isinstance(string, str):
        strings = string
    elif not strings:
        strings = (string,)
    else:
    ...
    for string in strings:
        *path, obj = string.strip(".").strip(os.path.sep).split(os.path.sep)

        for subString in obj.split("."):
            if subString != "":
                obj = subString
                break
        parts.append((w for p in path + obj for w in p.split("_"))[::-1])
    
    words = []
    for part in zip(*parts):
        word = align(part, gapSymbol="-", missMatchSymbol="_", trim=True, compressed=True)
        score = (len(word.replace("_", ""))) / (sum(map(len, part)) / len(part))
