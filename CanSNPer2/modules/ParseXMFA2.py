		...
		for i, r in enumerate(self._ranges):
			if pos > r.start:
				if pos > r.stop:
					...
					return self._filepos[i] + pos - r.start + len(self._newline)*(pos - r.start)//80
		return -1
	


class ParseXMFA(object):
	entryHeader : re.Pattern = re.compile("^[>] [0-9]+:(?P<start>[0-9]+)-(?P<stop>[0-9]+) - .*", re.MULTILINE)
	filename : str
	coverage : Map

	def __init__(self, filename : str):
		self.filename = filename
		self.file = open(self.filename, "r+") # Makes the file writeable, but we do not intend to edit it
	...
	def mapBlocks(self):
		for m in self.entryHeader.finditer(mmap.mmap(self.file.fileno(), 0)):
			print(m)
			print(m.end())
			print(m.group(1))
			print(m.group(2))
